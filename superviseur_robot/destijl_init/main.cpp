...
/**
 * \fn void startTasks(void)extern RT_MUTEX mutex_cpt_err;
...
void initStruct(void) {
    ...
    if (err = rt_mutex_create(&mutex_cpt_err,NULL)) {
        printf("Error mutex create: %s\n",strerror(-err));
        exit(EXIT_FAILURE);
    }
    ...
    if (err = rt_task_create(&th_gestionBatterie, "th_gestionBatterie", 0, PRIORITY_TGESTIONBATTERIE, 0)) {
        printf("Error task create: %s\n", strerror(-err));
        exit(EXIT_FAILURE);
    }
...
void startTasks() {
    ...
    if (err = rt_task_start(&th_gestionBatterie, &f_gestionBatterie, NULL)) {
        printf("Error task start: %s\n", strerror(-err));
        exit(EXIT_FAILURE);
    }
    if (err = rt_task_start(&th_gestionWatchDog, &f_gestionWatchDog, NULL)) {
        printf("Error task start: %s\n", strerror(-err));
        exit(EXIT_FAILURE);
    }
